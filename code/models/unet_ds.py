...
class unet_3D_ds_dist(nn.Module):
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3, is_batchnorm=True):
        super(unet_3D_ds_dist, self).__init__()
        self.is_deconv = is_deconv
        self.in_channels = in_channels
        self.is_batchnorm = is_batchnorm
        self.feature_scale = feature_scale
        ...
        self.dsv1 = nn.Conv3d(
            in_channels=filters[0], out_channels=n_classes, kernel_size=1)
        self.dsv1_dist = nn.Conv3d(
            in_channels=filters[0], out_channels=1, kernel_size=1)
    ...
    def forward(self, inputs):
        ...
        dsv1_dist = self.dsv1_dist(up1)
        if not self.training:
            return dsv1

        return dsv1, dsv2, dsv3, dsv4 , dsv1_dist
