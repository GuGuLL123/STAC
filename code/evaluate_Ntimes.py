...
if __name__ == '__main__':
    ...
    txt_path = "./logs/"+args.exp+"/evaluation_stac_{}_length_{}_{}_{}_seed_{}.txt".format(str(args.use_stac),str(args.alpha),str(args.bata),str(args.max_extent),str(args.seed))
    ...
    for fold in range(1, args.folds+1):
        ...
        for idx, data_id in enumerate(tqdm(ids_list)):
            ...
            pred = read_nifti(os.path.join("./logs",args.exp, "fold"+str(fold)+"_stac_{}_length_{}_{}_{}_seed_{}".format(str(args.use_stac),str(args.alpha),str(args.bata),str(args.max_extent),str(args.seed)), "predictions_"+str(args.cps),f'{data_id}.nii.gz'))
            if args.task == "amos":
